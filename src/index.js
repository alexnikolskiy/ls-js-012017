...
function findAllPSiblings(where) {
    ...
    for (let child of where.childNodes) {
        if (child.childNodes.length) {
            arr = arr.concat(findAllPSiblings(child));
        } else if (child.nextSibling && child.nextSibling.tagName == 'P') {
            arr.push(child);
        }
    }
    ...
    let mo = new MutationObserver((records) => {
        records.forEach((record) => {
            if (record.addedNodes.length) {
                fn({ type: 'insert', nodes: [...record.addedNodes] });
            }
            if (record.removedNodes.length) {
                fn({ type: 'remove', nodes: [...record.removedNodes] });
            }
        })
    });
